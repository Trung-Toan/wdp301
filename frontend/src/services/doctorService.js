...
export const getDashboardStats = async (doctorId) => {
  ...
  const response = await fetch(`${BASE_URL}/doctor/${doctorId}/dashboard/stats`)
...
export const getTodayAppointmentsList = async (doctorId) => {
  if (USE_MOCK_DATA) {
    await delay()
    const appointments = getTodayAppointments().filter((apt) => apt.doctor._id === doctorId)
    return {
      success: true,
      data: appointments,
    }
  ...
  const response = await fetch(`${BASE_URL}/doctor/${doctorId}/appointments/today`)
  return response.json()
}
...
export const getPatients = async (doctorId, filters = {}) => {
  if (USE_MOCK_DATA) {
    ...
    const doctorAppointments = mockAppointments.filter((a) => a.doctor_id === doctorId)
    const patientIds = [...new Set(doctorAppointments.map((a) => a.patient_id))]
    let patients = patientIds.map((id) => getPopulatedPatient(id)).filter(Boolean)

    // Apply filters
    if (filters.search) {
      patients = patients.filter((p) => p.user.full_name.toLowerCase().includes(filters.search.toLowerCase()))
    }
  ...
  const response = await fetch(`${BASE_URL}/doctor/${doctorId}/patients?${queryParams}`)
  return response.json()
}
...
export const getAppointments = async (doctorId, filters = {}) => {
  ...
  const queryParams = new URLSearchParams(filters).toString()
  const response = await fetch(`${BASE_URL}/doctor/${doctorId}/appointments?${queryParams}`)
  return response.json()
}
...
export const getSlotsByDate = async (doctorId, date) => {
  if (USE_MOCK_DATA) {
    await delay()
    const slots = getSlotsByDoctorAndDate(doctorId, date)
    return {
      success: true,
      data: slots,
      total: slots.length,
  ...
  const response = await fetch(`${BASE_URL}/doctor/${doctorId}/slots?date=${date}`)
  return response.json()
}
...
export const getPrescriptions = async (doctorId, filters = {}) => {
  if (USE_MOCK_DATA) {
    await delay()
    let records = mockMedicalRecords.filter((r) => r.doctor_id === doctorId && r.prescription)
  ...
  const queryParams = new URLSearchParams(filters).toString()
  const response = await fetch(`${BASE_URL}/doctor/${doctorId}/prescriptions?${queryParams}`)
  return response.json()
}
...
export const verifyPrescription = async (medicalRecordId, doctorId) => {
  ...
  const response = await fetch(`${BASE_URL}/medical-records/${medicalRecordId}/prescription/verify`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export const rejectPrescription = async (medicalRecordId, reason) => {
  ...
  const response = await fetch(`${BASE_URL}/medical-records/${medicalRecordId}/prescription/reject`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export const getMedicalRecordRequests = async (doctorId, filters = {}) => {
  ...
  const queryParams = new URLSearchParams(filters).toString()
  const response = await fetch(`${BASE_URL}/doctor/${doctorId}/access-requests?${queryParams}`)
  return response.json()
}

export const requestMedicalRecordAccess = async (doctorId, patientId, medicalRecordId) => {
  if (USE_MOCK_DATA) {
    await delay()
    return {
      success: true,
  ...
  const response = await fetch(`${BASE_URL}/medical-records/${medicalRecordId}/access-request`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ doctor_id: doctorId }),
  })
...
export const respondToAccessRequest = async (medicalRecordId, doctorId, response, notes) => {
  if (USE_MOCK_DATA) {
    await delay()
    return {
      success: true,
      message: `Yêu cầu đã được ${response === "APPROVED" ? "chấp nhận" : "từ chối"}`,
    }
  }

  const res = await fetch(`${BASE_URL}/medical-records/${medicalRecordId}/access-request/${doctorId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export const getFeedback = async (doctorId, filters = {}) => {
  if (USE_MOCK_DATA) {
    ...
    if (filters.rating) {
      feedback = feedback.filter((f) => f.rating === Number.parseInt(filters.rating))
    }
  ...
  const queryParams = new URLSearchParams(filters).toString()
  const response = await fetch(`${BASE_URL}/doctor/${doctorId}/feedback?${queryParams}`)
  return response.json()
}
